...
#endif
    //[MC] 对当前AutoreleasePoolPage 完整性的校验
	magic_t const magic;
    //[MC] 指向下一个即将产生的autoreleased对象的存放位置
    //（当next == begin()时，表示AutoreleasePoolPage为空；
    // 当next == end()时，表示AutoreleasePoolPage已满
	__unsafe_unretained id *next;
    //[MC] 当前线程，表明与线程有对应关系
	pthread_t const thread;
    //[MC] 指向父节点，第一个节点的 parent 值为 nil；
	AutoreleasePoolPage * const parent;
    //[MC] 指向子节点，最后一个节点的 child 值为 nil；
	AutoreleasePoolPage *child;
    //[MC] 代表深度，第一个page的depth为0，往后每递增一个page，depth会加1；
	uint32_t const depth;
    //[MC] 表示high water mark（最高水位标记）
	uint32_t hiwat;
