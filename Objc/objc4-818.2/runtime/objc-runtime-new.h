...
struct class_rw_t {
    ...
    /* explicit_atomic是继承自C++的atomic，里面封装了一个值的类型，它的访问保证不会导致数据的竞争，并且可以用于在不同的线程之间同步内存访问 */
    /* 原子类型数据，存储着class_rw_ext_t或class_ro_t指针 */
...
private:
    /*
     ro_or_rw_ext 是指针，这个指针指向了class_rw_ext_t或class_ro_t
     ro_or_rw_ext_t 是类,类里存储着class_rw_ext_t或class_ro_t 结构体
     */
    
    /*PointerUnion
     一个封装可同时兼容class_rw_ext_t、class_ro_t类型数的类，就是可以存储要么是class_rw_ext_t类型的数据，要么是class_ro_t类型的数据，不能同时存储，鱼和熊掌不可兼得，其中有一个is()方法，用来判断是某一个特性类型的数据，get()方法，获取特性类型的数据。*/
    /* 别名操作，类似typedef */
...
struct objc_class : objc_object {
  ...
  void operator=(objc_class&&) = delete;
    ...
    class_data_bits_t obbits;    // class_rw_t * plus custom rr/alloc flags
