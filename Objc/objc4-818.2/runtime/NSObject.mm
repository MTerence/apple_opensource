...
#   define POOL_BOUNDARY nil
    ...
    //[MC] stop 的值即为最初push时返回的哨兵对象的地址.
    void releaseUntil(id *stop) 
    {
        // Not recursive: we don't want to blow out the stack 
        // if a thread accumulates a stupendous amount of garbage
        // [MC] 循环依次向autorelease对象发送release消息
        while (this->next != stop) {
            // [MC] AutoreleasePoolPage 有cold和hot之分.hot是当前正在使用的,cold是没有使用的
            // 获取当前正在使用的
            ...
            // [MC]如果为空,通过parent指针指向它的父节点,并将父节点置为当前使用的page
            while (page->empty()) {
                page = page->parent;
                setHotPage(page);
            }
            ...
            if (obj != POOL_BOUNDARY) {
                ...
                for (int i = 0; i < count + 1; i++) {
                    // [MC]从next的上一个元素开始,向上查找只要不是哨兵对象,就向其发送release消息
                    objc_release(obj);
                }
#else
                objc_release(obj);
